...
fn progress_range_pair(
    pair: (&Range<u64>, &Range<u64>), input_map: &InputMap
...
fn progress_range_map(
    current_range_map: RangeMap, input_data: &InputData
) -> RangeMap {
    let mut range_mapping = HashMap::<Range<u64>, Range<u64>>::new();
    let relevant_input_map = input_data.maps
        .iter()
...
fn seedrange_to_locationrange(input_data: InputData) -> RangeMap {
    ...
    let mut range_map = RangeMap {kind, mapping: initial_range_map};
...
fn parse_seed_ranges_from_input(seed_description: &str) -> Vec<Range<u64>> {
    let numbers: Vec<u64> = seed_description
        .split(" ")
        .collect::<Vec<&str>>()[1..]
        .iter()
        .map(|s|s.parse::<u64>().unwrap())
    ...
    zip(&numbers[..], &numbers[1..])
        .enumerate()
        .filter(|(i, _)| (i % 2) == 0)
        .map(|(_, (this, next))| *this..(this + next))
        .collect()
}
...
fn parse_input(filename: &str) -> InputData {
    ...
    match &puzzle_input.split("\r\n\r\n").collect::<Vec<&str>>()[..] {
        [unparsed_seeds, unparsed_maps @ ..] => {
            ...
            let maps: Vec<InputMap> = unparsed_maps.iter()
                .map(parse_map_from_input)
                .collect();
